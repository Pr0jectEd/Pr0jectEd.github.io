...
class Vehicle {
	...
	public function __toString() {	// to print the object
		return "Vehicle: " . $this->_name . " is now at position: " . $this->_currPos ." and distance is:".$this->_currDist. "<br/>";
	}

		// 3/ public methods : just necessary things invoked from outside
	public function multipleMove(int $nb=2) : void {
		for ($i=0; $i < $nb; $i++) {
			$dist = $this->randVehicle();
			echo $this->_name . ' move from: ' . $dist . '...<br/>';
            file_put_contents("movement_log.txt", $this->_name . ' move from: ' . $dist . '...<br/>'. "\n", FILE_APPEND);
			$this->_currPos += $dist;
...
//file_put_contents("calculator_log.txt", $operation.$a.$b. "\n", FILE_APPEND);
